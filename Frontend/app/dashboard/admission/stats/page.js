...
export default function AdmissionSettingsCard() {
	...
	const handleToggleAdmission = async () => {
		...
		try {
			...
			setError(err.response?.data?.message || "Could not update admission status.");
		} finally {
			setLoading(false);
		}
	};
	...
	const handleFeeUpdate = async () => {
		...
		try {
			...
			setError(err.response?.data?.message || "Could not update admission fee.");
		} finally {
			setLoading(false);
		}
	};
				...
				<div>
					{isAdmissionOpen === null ? (
						...
						<button
							...
							${!isAdmissionOpen ? "bg-green-600 hover:bg-green-700" : "bg-red-600 hover:bg-red-700"}`}
						>
							{loading
								? "Updating..."
								: isAdmissionOpen
