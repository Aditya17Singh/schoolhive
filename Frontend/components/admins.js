...
export default function Admins() {
  ...
  const [error, setError] = useState(""); // also needed if you're using setError
	...
	// Fetch admins from backend on load
	useEffect(() => {
		fetchAdmins();
	}, []);
	...
	const fetchAdmins = async () => {
		try {
			...
			setAdmins(
				data.map((admin) => ({
					id: admin._id,
					orgId: admin.orgId.code || admin.orgId, // depends on populated data
					name: `${admin.firstName} ${admin.middleName || ""} ${admin.lastName}`.trim(),
					email: admin.email,
					phone: admin.phone,
					permissions: [], // or whatever you have
				}))
			);
		} catch (error) {
			console.error(error);
	...
	// Filter admins by search query (case-insensitive)
	const filteredAdmins = admins.filter((admin) =>
		admin.name.toLowerCase().includes(search.toLowerCase())
	);

	// Check if all visible admins are selected
	const allSelected =
		filteredAdmins.length > 0 &&
		filteredAdmins.every((admin) => selectedIds.has(admin.id));

	// Handle parent checkbox toggle (select/unselect all)
	...
	// Delete button enabled if at least one selected
	const isDeleteEnabled = selectedIds.size > 0;

	// Placeholder delete handler (implement your logic here)
	const handleDelete = () => {
		alert(`Deleting Admins with IDs: ${[...selectedIds].join(", ")}`);
	...
	const handleSubmit = async (e) => {
		...
		// For now, ignoring profilePhoto upload
		...
		try {
			...
			// Success: Refresh admins list & close dialog
			await fetchAdmins();
			setOpen(false);
			form.reset();
		} catch (err) {
			...
			<div className="flex flex-col sm:flex-row justify-between items-start gap-4">
				...
				<div>
					<button
						...
						className={`flex items-center gap-1 whitespace-nowrap rounded-md px-4 py-2 text-sm font-medium shadow-sm transition ${isDeleteEnabled
					...
					<tbody>
						...
						{filteredAdmins.map((admin) => (
							...
							>
								...
								<td className="p-2">{admin.orgId}</td>
