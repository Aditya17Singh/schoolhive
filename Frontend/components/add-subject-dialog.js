...
export function AddSubjectsDialog({
  ...
  useEffect(() => {
    if (open) {
      setSelected(classSubjectNames);
    }
  }, [open, JSON.stringify(classSubjectNames)]);
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 z-50 w-[90%] max-w-4xl -translate-x-1/2 -translate-y-1/2 bg-white p-6 rounded-lg shadow-lg space-y-4 max-h-[90vh] overflow-y-auto">
                ...
                {selected.map((subject) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="ml-2 text-blue-600 hover:text-red-500 cursor-pointer"
                ...
                <div className="flex flex-wrap gap-2">
                  {subjects.map((subject) => (
                    <button
                      ...
                      className={`px-4 py-2 cursor-pointer rounded-lg text-sm font-medium transition-all border flex items-center gap-2 ${
          ...
          <div className="flex justify-end space-x-2 pt-4">
            <Dialog.Close asChild disabled={loading}>
              <button className="px-4 py-2 cursor-pointer border rounded hover:bg-gray-100">
                Cancel
              </button>
            </Dialog.Close>
            <button
          ...
          <Dialog.Close asChild>
            <button className="absolute top-4 right-4 text-lg cursor-pointer">
              ✕
            </button>
          </Dialog.Close>
        </Dialog.Content>
