...
import { format, parse, startOfWeek, getDay, format as formatDateFns } from 'date-fns';
...
const CalendarSchedule = () => {
	...
	const [form, setForm] = useState({ title: '', description: '', type: 'event', start: '', end: '' });

	const user = JSON.parse(localStorage.getItem('user'));

	const fetchEvents = async () => {
		try {
			const res = await axios.get(`http://localhost:5000/api/schedule/${user.id}`, {
	...
	const handleSelectSlot = ({ start }) => {
		...
		setSelectedEvent(null); // Creating new event
		setForm({ title: '', description: '', type: 'event', start: localStart, end: localEnd });
		setModalOpen(true);
	};
